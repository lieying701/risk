 ...
 * according to the "fixedRate", "fixedDelay", or "cron" expression provided via
 * the annotation.
 * <p> 1
 * <p> 1
...
public class ScheduledAnnotationBeanPostProcessor
        implements MergedBeanDefinitionPostProcessor, DestructionAwareBeanPostProcessor, Ordered,
     ...
     * invoke the scheduled methods, or a {@link ScheduledExecutorService} to be
     * wrapped as a TaskScheduler.
     * <p>
     * If not specified, default scheduler resolution will apply: searching for
     * a unique {@link TaskScheduler} bean in the context, or for a
     * {@link TaskScheduler} bean named "taskScheduler" otherwise; the same
     * lookup will also be performed for a {@link ScheduledExecutorService}
     * bean. If neither of the two is resolvable, a local single-threaded
     * default scheduler will be created within the registrar.
     *
    ...
    private <T> T resolveSchedulerBean(Class<T> schedulerType, boolean byName) {
        if (byName) {
            ...
            return scheduler;
        } else if (schedulerType == TaskScheduler.class) {
            return this.beanFactory.getBean("defaultSockJsTaskScheduler", schedulerType);
