cola(function (model) {
    ...
    //设置投保人
    var policyHolder = model.get("actuals.plcApplicant").toJSON();//投保人
    if( policyHolder ){
        ...
        if (model.get("policyHolder.historyPolicy")) {
            //时间格式转换
            model.get("policyHolder.historyPolicy").each(function (entity) {
        ...
        if (model.get("policyHolder.historyClaims")) {
            model.get("policyHolder.historyClaims").each(function (entity) {
                var claimRptTime = XDate(entity.get("claimRptTime")).toString("yyyy-MM-dd");
                var claimEndTime = XDate(entity.get("claimEndTime")).toString("yyyy-MM-dd");
    ...
    //设置被保人
    var proposalNo=model.get("actuals.proposalNo");
    model.describe("searchPlcInsurant", {
        provider: {
            url: "controller/policy/groupInsurant/listGroupQueryBoss?from={{$from}}&limit={{$limit}}",
            pageSize: 10,
            sendJson: true,
            parameter: {
                businessNo: proposalNo,
                customerName: "",
                identifyNumber: ""
            },
            ajaxOptions: {
            ...
            response: function (self, arg) {
                ...
                var plcInsurantList = result.data$;
                if (!plcInsurantList) {
                    result.data$ = [];
                    return;
                }
                ...
                //开始处理被保人数据
                model.set("plcInsurantList",[]);
                //循环设置被保人数据
                for( var i=0;i<plcInsurantList.length;i++ ){
                    var paramsInsurant = {
                        idType: plcInsurantList[i].identifyType,
                        idNo: plcInsurantList[i].identifyNumber,
                        idName: plcInsurantList[i].customerName
                    };
                            ...
                            if( data.responseCode != 0 ){
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        plcInsurantList[i].customerLevel = data.custRiskLevel.manualRiskLevelCode?data.custRiskLevel.manualRiskLevelCode:data.custRiskLevel.autoRiskLevelCode;
                                    }
                                });
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        //设置当前投保人的历史保单
                                        plcInsurantList[i].historyPolicy = data.policys;
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        //设置当前投保人的历史理赔保单
                                        plcInsurantList[i].historyClaims = data.claims;
                                    }
                                })
                            }else{
                                cola.NotifyTipManager.warning({
                ...
                model.set("plcInsurantList", plcInsurantList);//设置被保人
                if(model.get("plcInsurantList.historyPolicy")){
                    //时间格式转换
                    model.get("plcInsurantList.historyPolicy").each(function (entity) {
                        var start=XDate(entity.get("startTime")).toString("yyyy-MM-dd");
                        var endTime = XDate(entity.get("endTime")).toString("yyyy-MM-dd");
                        entity.set("startTime",start);
                        entity.set("endTime",endTime);
                ...
                if(model.get("plcInsurantList.historyClaims")){
                    model.get("plcInsurantList.historyClaims").each(function (entity) {
                        var claimRptTime = XDate(entity.get("claimRptTime")).toString("yyyy-MM-dd");
                        var claimEndTime = XDate(entity.get("claimEndTime")).toString("yyyy-MM-dd");
                        entity.set("claimRptTime",claimRptTime);
                        entity.set("claimEndTime",claimEndTime);
    ...
    model.action({
        //设置投保人投保/理赔历史
        setPlcApplicantHistory: function (item) {
            console.log("查看当前投保人历史");
            model.set("historyPolicyApplicant",item.historyPolicy);//设置投保历史
            model.set("historyClaimsApplicant",item.historyClaims);//设置理赔历史
        },
        //设置被保人投保/理赔历史
        setPlcIansurantHistory: function (item) {
            console.log("查看当前被保人历史");
            model.set("historyPolicyInsurant",item.historyPolicy);//设置投保历史
            model.set("historyClaimsInsurant",item.historyClaims);//设置理赔历史
        }
    });
